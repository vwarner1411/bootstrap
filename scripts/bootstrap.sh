...
run_chezmoi() {
  ...
  chez_cmd+="   if [ -n \"\$branch\" ] && chezmoi git -- show-ref --verify --quiet \"refs/remotes/origin/\${branch}\"; then"
  chez_cmd+="     if [ -z \"\$status\" ]; then"
  chez_cmd+="       echo '[chezmoi] Resetting to origin/'\"\$branch\" >&2;"
  chez_cmd+="       chezmoi git -- reset --hard \"origin/\${branch}\" >/dev/null 2>&1 || true;"
  chez_cmd+="     else"
  chez_cmd+="       echo '[chezmoi] Local changes detected; skipping git reset' >&2;"
  chez_cmd+="     fi;"
  chez_cmd+="   fi;"
